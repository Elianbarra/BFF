...
const STAFF_ROLES = ["DOCTOR", "NURSE", "ADMIN", "RECEPTIONIST"] as const;
...
export default function StaffRegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(
        `${ROLE_LABELS[form.role]} ${data.firstName} ${data.lastName} registrado correctamente.`
      );
      resetForm();
    } catch {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <nav className="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between shadow-sm">
        ...
        <Link href="/dashboard" className="text-sm text-blue-700 hover:underline font-medium">
          ← Volver al dashboard
        </Link>
      </nav>
